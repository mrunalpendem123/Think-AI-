...
export async function POST(req: NextRequest) {
  try {
    ...
    const { messages, model, temperature, check_health } = body;
    ...
    const response = await fetch(OLLAMA_URL, {
      ...
      body: JSON.stringify({
        ...
        messages,
        stream: !check_health, // Only stream if not a health check
        temperature: temperature || 0.7,
        check_health // Pass this through!
      }),
    });
    ...
    console.error("Local LLM Proxy connection failed.");
    console.error("Target URL:", "http://127.0.0.1:11434/v1/chat/completions");
